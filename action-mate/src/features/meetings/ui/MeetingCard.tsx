...
export function MeetingCard({ item }: { item: MeetingPost }) {
  ...
  const isJoinBlocked = !isClosed && !item.myState?.canJoin && !isHost && !isMember && item.status !== "STARTED";
  ...
  const disabledStyle = useMemo(() => {
    ...
    if (item.status === "FULL") {
      return { bg: withAlpha(t.colors.warning, t.mode === "dark" ? 0.10 : 0.06), border: withAlpha(t.colors.warning, t.mode === "dark" ? 0.32 : 0.22), opacity: 0.9, title: withAlpha(t.colors.textMain, 0.78) };
    }
  ...
  // ✅ [수정 완료] 안전한 데이터 접근 (Nested Object 대응)
  // item.location.name이 없으면 빈 문자열 ("") 대신 "장소 미정" 출력 등
  const locationName = item.location?.name || "장소 미정"; 
  const distanceText = item.distanceText ?? "";
  
  // ✅ [수정 완료] capacity 객체 접근
    ...
    >
      ...
      {item.content ? (
        <View style={[styles.memoRow, { borderTopColor: t.colors.divider ?? t.colors.border }]}>
          <Ionicons name="chatbubble-ellipses-outline" size={14} color={isDisabled ? iconMuted : iconDefault} style={{ marginTop: 2 }} />
...
const styles = StyleSheet.create({
  ...
  memoRow: { marginTop: 12, flexDirection: "row", alignItems: "flex-start", gap: 6, paddingTop: 10, borderTopWidth: 1 },
});