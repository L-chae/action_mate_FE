...
function validate(form: Form) {
  ...
  if (!isValidEmail(email))
    return "이메일 형식에 맞게 입력해 주세요. (예: abc@naver.com)";
    ...
    >
      <Text
        style={[
          ...
          { color: active ? t.colors.textMain : t.colors.textSub },
        ]}
      >
        {label}
      </Text>
...
export default function SignupPage() {
  ...
  // ✅ 모달 딤도 토큰 우선 + fallback은 textMain에 알파만
  const scrimToken =
    c.scrim ?? c.backdrop ?? c.overlay ?? c.dim ?? c.modalDim ?? t.colors.textMain;
  const scrim = withAlpha(scrimToken, 0.35);

  const activeBg =
    c.surfaceAlt ?? c.surface2 ?? c.card ?? t.colors.surface ?? t.colors.background;
  ...
  const birthText = form.birthDate
    ? formatYMD(form.birthDate)
    : "생년월일을 선택해 주세요.";
            ...
            <Card>
              ...
              <Text
                style={[
                  t.typography.labelMedium,
                  styles.label,
                  { marginTop: 12, color: t.colors.textSub },
                ]}
              >
                닉네임
              </Text>
              ...
              >
                <Text
                  style={[
                    t.typography.bodySmall,
                    { color: form.birthDate ? t.colors.textMain : t.colors.textSub },
                  ]}
                >
                  {birthText}
                </Text>
              ...
              {error ? (
                <Text style={[t.typography.bodySmall, { color: danger, marginTop: 10 }]}>
                  {error}
                </Text>
              ) : null}
